...
class SytwysGUIgrid( tk.Frame):
    ...
    def eventHandler_entry_obreb( self, event):
        ...
        if len( obr) >= 3:
            ...
            if obr in self.t.dictObr_nazwaObrWpisana2teryt:
                print( "Obręb %s jest w słowniku" % ( obr))
            else:
                print( "Obrębu %s NIE MA w słowniku" % ( obr))
            '''
            liczbaKluczy = 0
            lista = ""
            listaTerytow = []
    ...
    def zapisz( self):
        ...
        with open(  self.sw.sw_plikTytul_fullPath, "w") as f2:
            ...
            f2.write(   "Geodezyjny układ wysokościowy: PL-KRON86-NH" + "\n")
            ...
            if self.sw.sw_typ == "inw":
                ...
                f2.write( "z projektem zagospodarowania terenu (załącznik nr %s do decyzji\n" % ( self.sw.sw_inw_nrZal))
    ...
    def wczytaj( self):
        ...
        # - na razie za pomocą okna dialogowego
        # >> zrobić przekazywanie ścieżki pliku do otwarcia jako argumentu
        ...
        # godła i text sekcje
        # utworzenie listy sekcji i słownika sekcji
        #-------------------------------------------------------------------------
    ...
    def btn_gen_sekcje( self):
        ...
        if os.path.exists( self.sw.sw_plikGodla2swInfo_fullPath) == False:
...
#        self.tx_sekcje.insert( tk.END, self.sw.sw_sekcje_tytul)
    ...
    def btn_gen_uwagi2( self):
        ...
        # if self.sw.mdcp.ust5 == "1" and self.sw.mdcp.ust6 == "1":
        #     self.sw.mdcp.kp_uwagi2 += "spełniają warunki przepisów §79, ust. 5 i 6 \n"
        # elif self.sw.mdcp.ust5 == "1" and self.sw.mdcp.ust6 == "0":
        #     self.sw.mdcp.kp_uwagi2 += "spełniają warunki przepisów §79, ust. 5 \n"
        # elif self.sw.mdcp.ust5 == "0" and self.sw.mdcp.ust6 == "1":
        #     self.sw.mdcp.kp_uwagi2 += "spełniają warunki przepisów §79, ust. 6 \n"
        # else:
        #     self.sw.mdcp.kp_uwagi2 = self.sw.mdcp.kp_uwagi2 + "nie spełniają warunków przepisów §79, ust. 5 i 6 \n"

        self.sw.mdcp.kp_uwagi2 += " spełniają wymogi dokładnościowe dla znaków i punktów granicznych,"
        self.sw.mdcp.kp_uwagi2 += " określone w § 16 pkt 1 rozporządzenia Ministra Rozwoju"
